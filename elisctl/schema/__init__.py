...
def download_command(ctx: click.Context, id_: str, indent: int, ensure_ascii: bool, output_file: Optional[IO[str]]):
    with APIClient() as api_client:
        schema_dict = get_json(api_client.get(f"schemas/{id_}"))
    schema_json = json.dumps(schema_dict["content"], indent=indent, ensure_ascii=ensure_ascii, sort_keys=True)
    if output_file:
